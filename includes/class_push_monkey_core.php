...
class PushMonkey { 
	...
	function add_actions() {
		...
		add_action( 'admin_init', array( $this, 'pm_flush_rewrite_rules' ) );
		...
		add_action( 'wp_head', array( $this, 'sw_meta' ) );
	...
	*/
	function pm_flush_rewrite_rules() {

		if ( get_option( self::FLUSH_REWRITE_RULES_FLAG_KEY ) == true ) {

		  $account_key = get_option( self::ACCOUNT_KEY_KEY, NULL );
		  if ( $account_key ) {
        ...
        $wp_rewrite->flush_rules( true );
			...
			update_option( self::FLUSH_REWRITE_RULES_FLAG_KEY, false);
		}
	}
...
<?php
		...
		echo '<a href="http://www.getpushmonkey.com/help?source=plugin#gpm4">What is this?</a>';
	}
	...
	function settings_screen() {
		...
		if ( $this->signed_in() ) {
			...
			if ( $welcome_status != false ) {

				$this->d->debug("we have a welcome status");
				$this->d->debug($welcome_status["message"]);				
				$this->d->debug(print_r($welcome_status, true));
		...
		$segment_delete_url = admin_url( 'admin.php?page=push_monkey_main_config&delete_seg=' );
	...
	function post_published( $new_status, $old_status, $post ) {
        ...
        $uploads_info = wp_upload_dir();
			...
			$post_id = $post->ID;
			$this->send_push_notification( $title, $body, $post_id, false, $segments, $image );
		}
	} 
   ...
   * Return: array of paths to the found files. 
	 ...
	 */
	function send_push_notification( $title, $body, $url_args, $custom, $segments, $image = NULL ) {
		...
		$this->apiClient->send_push_notification( $account_key, $title, $body, $url_args, $custom, $segments, $image );
	}
	...
	*/
	function sw_meta( ) {
	    ...
	    echo '<link rel="manifest" href="' . plugins_url( 'assets/manifest.json', plugin_dir_path( __FILE__ ) ) . '">';
	}
	...
	function process_push( $post, $files ) {
		...
		$this->send_push_notification( $title, $body, $url_args, true, $segments, $image );
	...
	function segment_saved_notice() {
		...
		echo '<div class="updated"><p>Segment saved! *woohoo*</p></div>';	
	}
	...
	function welcome_message_notice() {
		...
		echo '<div class="updated"><p>Welcome message saved! *woohoo*</p></div>';	
	}	
	...
	function segment_saving_error_notice() {
		...
		echo '<div class="notice notice-error"><p>Error saving segment. Please try again.</p></div>';			
	}
	...
	function segment_deleted_notice() {
		...
		echo '<div class="updated"><p>Segment deleted! *boom*</p></div>';	
	}
	...
	function segment_deleting_error_notice() {
		...
		echo '<div class="notice notice-error"><p>Error deleting segment. Please try again.</p></div>';			
	}	
